...
export const CITY_COST = 40;       // was 50 — easier to build early
export const CITY_INCOME = 30;     // was 20 — cities are more valuable
export const SOLDIER_COST = 8;     // was 10 — recruiting is cheaper
 ...
 *
 * Formula: base = 8 + turn * 3   (was 10 + turn * 5 — much gentler scaling)
 * Random spread: -2..+4           (was 0..10 — less swingy)
 */
export function estimateEnemyPower(turnNumber) {
  const base = 8 + turnNumber * 3;
  return { min: base - 2, max: base + 4, midpoint: base + 1 };
}
...
export function threatLevel(turnNumber) {
  ...
  if (mid < 20) return 'Low';
  if (mid < 35) return 'Moderate';
  if (mid < 55) return 'High';
  return 'Critical';
}
